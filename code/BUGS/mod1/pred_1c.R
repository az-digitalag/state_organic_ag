...
y <- readr::read_csv('../../../raw_data/organic_area_1997.csv',
                     col_types = cols(
                       state = col_character(),
                       number = col_number(),
                       cropland = col_number(),
                       pasture_rangeland = col_number(),
                       total = col_number()
                     )) %>%
  mutate(year =  1997 - 2000,
         stateID = 1:50,
         farm_sales = NA,
         farm_ha = ud.convert(total, "acres", "ha")) %>%
  rename(farm_number = number) %>%
  select(year, state, farm_number, farm_ha, farm_sales, stateID)
...
# Add 1997 data, but remove farm_ha for Alabama, remove farm_number for all states
dat <- rbind.data.frame(y, dat)
dat$farm_ha[dat$state == "Alabama" & dat$year == "-3"] <- NA
dat$farm_number[dat$year == "-3"] <- NA
...
save(coda_rep, file = "coda/pred_coda_1c.Rdata")
# load("coda/pred_coda_1c.Rdata")
...
for(r in rgs){
  ...
  jpeg(filename = paste0("figs_1c/preds/fig_pred_", r, ".jpg"), 
...
# For 1997, change number and sales to NA
datn$obs.sum[datn$year == -3 & datn$type %in% c("farm_number", "farm_sales")] <- NA
...
jpeg(filename = "figs_1c/preds/fig_pred_natl.jpg", 
     height = 3, width = 10, units = "in",
     res = 600)
print(fig_predn)
dev.off()
